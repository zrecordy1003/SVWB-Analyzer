...
**J-2 一個 Deck 對應幾份卡表？** ✅ 已決定：**(b) 保留版本歷史**，實作方式與理由見
`docs/deck-versioning-plan.md`（該文件是這個決定的唯一計畫來源）。做法是讓 Deck 那一列在
有對局引用之後永不改動、編輯時自動 fork 出新版本，並用 `familyId` 串起歷代版本。

原本擔心的「(b) 會讓 `Match.my_deckId` 的語意變模糊（指向哪個版本？）」在這個模型裡不成立
——它反而更精確：永遠指一份確定的卡表。階段 A～D 實作的是 (a)，所以在 011 上線之前被編輯
或重新匯入覆蓋過的牌組，舊卡表已經失去且追不回來。
...
- 此時 J-2（版本歷史）值得重新評估——能編輯就會產生版本
  → 已評估完畢並另立計畫：`docs/deck-versioning-plan.md`
...
> **設定頁的開關已於後續移除**（2026-09-01）：`settings.cardImages` 這個鍵、protocol
> handler 對它的判斷、以及關掉之後退成色塊／文字網格的所有路徑都還在，只是不再有 UI。
> 也就是說「一鍵關閉」現在要改設定檔才辦得到。若 Cygames 真的撤回許可，讓這件事重新
> 一鍵可達的做法是把開關加回設定頁，而不是動 handler。
