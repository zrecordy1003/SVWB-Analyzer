...
> 狀態：**部分實作**。P1、P2、P4（provenance、編輯覆蓋層、統計呈現，純本機）已完成；
> P5 之後全部未定案。
> 「判斷題」一節有 8 項需要在動工前決定，其中 J-1 決定其餘各項。
> 在 J-1 定案前，路線圖 P4 之後不應開工。
...
`src/main/telemetry/rollup.ts`：`(matches, watermark) -> { buckets, tiers, abandoned_count }`。
純函式，不碰網路。用既有 vitest 測，不需要 jsdom —— 正好落在 roadmap 說的「host 層零測試」缺口。

完成條件：Tier 判定、UTC 日期切分、放棄場次計數各有測試。
...
### P5｜伺服器 ingest（需 J-1 至 J-6 全部定案）

Cloudflare Worker + D1 + KV。先只收 ranked、只收 Tier A，觀察量體與健康度檢驗是否成立。
Settings 需先有 opt-in 開關與說明文案（預設關閉）。

### P6｜公開圖表

需 P0b 的準確率數字、P5 的實際樣本量、以及樣本偏差的明確標註同時到位。
...
**J-1｜這份資料是給誰看的？** 決定其餘各項。
(a) 給使用者看的環境 meta —— 需要伺服器、防灌水、面對樣本偏差，錯了會被公開引用。
(b) 給維護者看的辨識品質回饋 —— 本機即可，零隱私風險，直接解掉 P0b。
(c) 兩者，分階段。
傾向 (c) 且先做 (b)，理由見 P0b。

**J-2｜pre-008 的舊資料怎麼處理？** 1.2.0 已發布，現有使用者的全部歷史都沒有 provenance 且無法
回溯。全歸 legacy 排除（乾淨，但可能丟掉大部分現有資料量），或有條件納入 Tier B？

**J-3｜上傳範圍？** 傾向只 ranked：cpu / custom / plaza 對環境分析無意義，只會放大 plaza probe
已知缺陷的影響。

**J-4｜Cygames 服務條款。** 蒐集並公開發布對戰統計是否踩線，需要專案擁有者確認（其他遊戲的類似
工具有被要求下架的先例）。

**J-5｜開源導致端點完全公開，是否接受？** 見 R-5。這不是能修的，只能接受前提並靠統計防禦。

**J-6｜免費額度需實查。** 本文件刻意不寫具體數字。架構結論（桶化 + 每人每日一列，量級遠低於任何
免費方案的門檻）不依賴確切額度，但動工前應實際查一次 Cloudflare 的 pricing。
...
**J-8｜舊日期的編輯要不要回補？** 需要 dirty-day 追蹤才用得上 D-4 的覆寫語意，否則「使用者修正
三個月前的對局」永遠不會反映到聚合。或者接受「超過 N 天的編輯不回補」。

---

## 已知缺陷與風險

- **樣本偏差無法消除。** 會安裝分析工具的玩家，段位與行為分佈都與母體不同；opt-in 又疊一層。
  只能標註，不能修。
