...
### 決定：納入上傳（2026-09-17，專案擁有者定案）

觸發這個決定的是一句話：**「起手的重點應該放在，對上特定職業該留哪幾張」**。那個功能
本機版做不出來，不是「久一點就有」——是永遠不會有：

| | 本機（巫師 420 場） | 彙總（117 個安裝、一個改版週期 90 天） |
| --- | --- | --- |
| 每個對位的場次 | 420 / 7 ≈ 60 | ≈ 823 |
| 一張 3-of 被發到 | 17（沒發到 43） | 228（沒發到 595） |
| 差值的 95% 區間 | **±28pp** | **±7.6pp** |

要本機壓到 ±5pp，單一對位需要約 1,900 場，也就是用同一個職業打 13,500 場。
**所以這條路的先後順序反過來了**：先前寫的「先讓本機版自己有用再說」對「留牌勝率」是對的，
對「按對手職業拆」則不成立。

#### 桶的形狀

> **⚠ 這一小節的形狀已被 `docs/mulligan-advisor-plan.md` 取代。** 下面這個 key 少了
> 「其餘三張」，而那是把「留下時勝率」從相關性變成因果的唯一條件化變數——少了它，
> 這個功能最好的版本永遠做不出來，而且資料補不回來。列數的估計也算錯了（見那份文件）。
> 這裡保留原文，因為那個推論的其他部分仍然成立。


沿用既有的 `buckets` 那套（每個安裝每天一組計數，`content_hash` 沒變就整天跳過），
新增一張表，key 是：
...
- **只上傳 `pre` 手牌**（發到的四張），不傳 `post`。發牌是隨機的，`post` 是選擇；
  勝率的估計靠前者，後者只用來標記那一格 `kept`。
- **`kept` 留著，但它不是拿來算勝率的。** 它算的是「母體有多常留這張」——純描述、
  不需要估計、沒有選擇偏誤，也是 HSReplay 唯一那個誠實的指標（Kept %）。
  「留下時勝率多少」仍然不做，理由見階段 4 那節。
- **不上傳牌組。** 彙總版的基準線來自 `(my_class, oppo_class)` 的整體勝率——
  也就是既有的 `buckets` 表——不需要知道每個人的牌表。這省掉的是這整件事裡
  隱私成本最高的一塊。
- 信任分層與既有的桶一模一樣：只有 `clean`、引擎自己記錄、沒被手改過的對局。

#### 量測：這會不會再撞一次寫入牆

D1 免費額度每天 10 萬列寫入，而 91 個安裝就爆過一次（成因是每次上傳重寫 14 天窗口，
已用 `content_hash` 修掉——現在只有內容變過的那天會重寫）。

| | 每個安裝每天的列數 |
| --- | --- |
| 現有的 `buckets` | 約 15 |
| 新增的起手桶（每天 20 場 × 4 張，去重後再 ×2 的 kept） | **約 140** |

117 個安裝 ≈ 每天 1.6 萬列，還很寬鬆；**1000 個安裝 ≈ 7 萬列，開始逼近**；
2000 個就過線。也就是**餘裕從十幾倍縮到約一點五倍**。超過之前要走
`docs/account-sync-plan.md` 已經選好的 R2 append log + 排程聚合，不要試圖在 D1 上加速。

#### k-匿名會讓早期幾乎全空

`/v1/meta` 對每格要求至少 5 個安裝。現在的格子是 49 個（職業 × 職業），新的格子是
職業 × 職業 × 卡片——量級差兩三百倍，所以**上線初期絕大多數格子會被抑制**。
這不是 bug，而且門檻不能為了好看而調降：單一使用者的「對上龍族留這張」等同於那個人的
牌組與戰績。頁面要能把「還沒有足夠的人」講清楚，而不是顯示成 0。

#### 兩道閘門，都還沒過

1. **J-4（Cygames 服務條款）** 仍未定案。蒐集是一回事，公開發布又是一回事。
2. **必須重新徵求同意，不能默默擴大。** 現在的開關徵求的是「計數桶」；
   起手手牌是**每一場的卡片清單彙總後的計數**，性質不同。使用者按下那個開關時
   同意的不是這件事。實作上這表示一個獨立的、預設關閉的選項，而不是把現有開關的
   涵蓋範圍偷偷變大。
