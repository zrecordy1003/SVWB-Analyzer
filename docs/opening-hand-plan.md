...
Last updated: 2026-09-11（同日兩次：幾何閘門、指紋與比對）
...
> **階段 2（卡片辨識）做到一半（2026-09-11）**：辨識本身可以了——幾何閘門（`card.rs`）、
> 指紋與比對（`fingerprint.rs`）都已落地並用真實錄影驗證。**還沒接上的是資料**：候選集要從
> `DeckCard` / 卡池讀、指紋要落 `CardArtSample`、結果要寫回 `MatchOpeningCard.cardId`。
> 在那之前 `cardId` 仍然一律是 NULL。
> 方法本身是量出來的（2026-09-10，見〈階段 2 的可行性量測〉）：原本規劃的「先讀卡面數字」
> 不可行，**插畫比對才是主判準**，〈辨識架構〉已依量測結果改寫。
...
3. ~~指紋與比對。~~ **已完成（2026-09-11）**：`tools/engine/src/fingerprint.rs`。
   ...
   - **判準是勝差不是分數**（`CARD_ART_MIN_MARGIN` 0.15、`CARD_ART_MIN_SCORE` 0.55）。暗場景
