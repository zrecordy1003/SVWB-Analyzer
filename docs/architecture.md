...
對手名牌只在建立那一刻裁得下來：它只畫在 versus 畫面上，對戰開始後就不再出現。存的是**圖不是
文字**（`tools/engine/src/nameplate.rs`），因為 OCR 只讀得準拉丁名字，日文與中文每次都錯一兩個
字，而錯一個字的名字比沒有名字更糟。CPU 對手沒有名牌，該欄位為 NULL 是正常狀態。
