...
/**
 * The write-skip, asserted rather than assumed.
 *
 * This is the whole reason `unchanged` is in the response. Skipping writes
 * fails silently in both directions - skip too much and a corrected day never
 * lands, skip too little and the free tier is still on fire - and neither shows
 * up in any status code. The first upload of a window has nothing stored, so
