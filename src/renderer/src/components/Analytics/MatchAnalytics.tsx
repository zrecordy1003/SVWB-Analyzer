...
type Match = {
  result: number | null // 1=win,0=lose,null=ongoing
...
const MatchAnalytics: React.FC<Props> = ({ data }) => {
  ...
  const winRateData = useMemo(() => {
    ...
    filtered.forEach((m) => {
      if (m.my_class in stats && m.result !== null) {
        ...
        if (m.result === 1) stats[m.my_class].win++
      }
    })
  ...
  const orderData = useMemo(() => {
    ...
    filtered.forEach((m) => {
      ...
      else if (m.play_order === 'second') second++
    })
    return { labels: ['先攻', '後攻'], values: [first, second] }
  }, [filtered])
