 ...
 *
 * This dialog is a doorway, not a destination. It used to resolve the code and
 * then show a preview - class, counts, card list, a name field, a save button -
...
export default function NewDeckDialog({
  ...
  open: boolean
  onClose: () => void
  /** Hands the freshly imported - or already-owned - deck to the builder. */
  onOpenDeck: (deckId: number) => void
  /** Hands over to the full builder with nothing in it; this dialog closes first. */
  ...
  return (
    <AppDialog open={open} onClose={onClose} busy={busy}>
      {error && (
