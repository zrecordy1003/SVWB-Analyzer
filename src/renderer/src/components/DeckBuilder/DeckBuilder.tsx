 ...
 * ＋浮動 label 的表單欄位——見 `CONTROL_SX` 的註解。名稱有上限（`DECK_NAME_MAX_LEN`），
 * 所以字數就寫在框裡右側；滿了才變色，打字的時候不必一直盯著它。
 *
 * 這裡不重複那個數字。它原本寫死成「只有 8 個字」，上限放寬到 20 之後就成了
 * 一句錯的說明——而註解裡的常數沒有東西會提醒你它過期了。
 */
